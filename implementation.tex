...
\section{Implementation Details}
{
    ...
    \subsubsection{Code Generation}
    {
        ...
        % - Functions are objects holding lambdas, to avoid needing a distinct anonymous class for each closure (as done
        %   by Scala etc?) https://stackoverflow.com/a/11658895/2193020. Requires bootstrap methods, adds complexity to
        %   the code generator. Scala is strict, Haskell is lazy so essentially every expression is a closure, number of
        %   classes would be massive. Details on how a function contains argument list and free variable list, to allow
        %   for partial application. Details of cloning before applying, to prevent partial applications from bleeding
        %   out.
        % - JVM instructions are variable length (lookupswitch padding)
        % - JVMSanitise
        % - Datatype implementation, tag/data fields.
        % - Literal implementations. int, BigInteger, char, string is a list of chars.
        ...
        \paragraph*{Heap Objects}
        {
            ...
            Datatype values, literals, and functions are all represented at runtime by values on the heap, and will be
            referred to as `objects': this intentionally overloads the terminology used by Java for an instance of a
            class, as the two concepts are essentially interchangeable here.
            ...
            The abstract \java{enter} method evaluates the object to WHNF and returns a reference to the result,
            and the \java{clone} method simply returns a shallow copy of the object. This method is critically for
            implementing function applications, described later.

            \textbf{Literals} are builtin types that can't be defined as an Haskell ADT, such as \haskell{Int}. Any such
            type is a subclass of the \java{Data} class, which is itself a subclass of the \java{HeapObject} class that
            a rather boring implementation of the abstract \java{enter} method. Any literal is already in WHNF, so
        ...
        \paragraph*{JVM Sanitisation}
        {

            Haskell allows variable names that aren't valid Java or JVM identifiers.

            \todo[inline]{Quote the spec for the valid idents?}
