...
\section{Type Checker}
{
    ...
    \subsection{Type Inference}
    {
        ...
        \item
        {
            ...
            Haskell in Haskell' \cite{THIH}). My compiler does not implement this check, which allows some programs to
            be accepted by the compiler which crash at runtime (such cases can often be fixed by adding explicit type
...
\section{Code Generation}
{
    ...
    \subsection{Hooks}\label{sec:hooks}
    {
        ...
        \monospace{_primNumIntAddImpl}, as described in Section \ref{sec:closures}. The implementation of
        \monospace{_primNumIntAddImpl} is provided in the hook definition, and simply forwards its arguments to the
        \java{_Int::add} function shown in Section \ref{sec:literals}. The functions generated by the hook are, at the
...
\section{Summary}
{
    ...
    \item
    {
        ...
        After typechecking, types include typeclass `contexts', eg.\ \haskell{Eq a => a -> a -> Bool}. These are
        ...
        formal parameters eg.\ \haskell{Eq a -> a -> a -> Bool}.
    ...
    \item
    {
        ...
        \monospace{Case}, and the only node representing memory allocation is \monospace{Let}.
...
\section{Repository Structure}
{
    ...
    \subsection{\monospace{compiler-exe}}
    {
        \begin{minipage}{0.7\textwidth}
        \vspace{0pt}
        The compiler executable is imaginatively named \haskell{compiler-exe}, and the single 97-line source file
        \monospace{Main.hs} resides in the \monospace{app} directory: it contains the application entry point and code
        for handling command-line arguments, before passing control over to the \monospace{compiler} library for the
