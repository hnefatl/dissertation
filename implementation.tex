...
\section{Implementation Details}
{
    ...
    \subsection{Code Generation}
    {
        ...
        \subsubsection{Notable Instructions}
        {
            ...
            JVB uses variable-length instructions, and also instructions where the length varies depending on their
            position in a function. The \monospace{nop} (no-op) instruction is a single byte, whereas the
            \monospace{goto} instruction is 3 bytes (the opcode followed by a two-byte operand forming the address to
            jump to).
            
            However, there are more complex instructions: the \monospace{lookupswitch} instruction has the following
            layout:
