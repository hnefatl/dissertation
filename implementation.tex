...
\section{Preprocessor}
{
    ...
    \subsection{Dependency Analysis}\label{sec:dependency-analysis}
    {
        ...
        \item
        {
            ...
            To avoid this, we treat binding declarations inside \haskell{instance} declarations as being free uses rather than binding uses (despite them defining an implementation), so that the instance declaration forms a dependence on the class declaration where the variables are bound, ensuring it is typechecked first.
        }
...
\section{Type Checker}
{
    ...
    \subsection{Definition of Types}
    {
        ...
        A simple type is represented as any tree of applications between type variables and type constants: these are types such as \haskell{Int -> Maybe Bool}. Haskell has more complex types, however: overloaded and polymorphic types.
    ...
    \subsection{Type Inference}
    {
        ...
        {
            As a defence against the issues detected by the ambiguity check, and to make Haskell code slightly cleaner, the Haskell Report defines a feature called defaulting\footnote{\url{https://www.haskell.org/onlinereport/decls.html\#sect4.3.4}} which allows for ambiguous type variables appearing in \haskell{Num} constraints to be defaulted to a ground type. 
...
\section{Lowering and Intermediate Languages}
{
    ...
    \subsection{Intermediate Language A}
    {
        ...
        \subsubsection{Literals are not patterns}
        {
            ...
            Pattern matches using literals are desugared into equality checks: the expression \haskell{case x of { 0 -> y ; 1 -> z ; _ -> w }} is essentially translated into the Haskell expression \haskell{if x == 0 then y else if x == 1 then z else w}.
        }
    }
...
\section{Repository Structure}
{
    ...
    {
        The vast majority of code written was in a library named \monospace{compiler}, which exposes an API for compiling Haskell programs -- the code for this library is within the \monospace{src} directory. \monospace{Compiler.hs} contains functions that string all of the pipeline stages together.

        This library contains 6344 lines of Haskell source code.
    }
    \subsection{\monospace{compiler-exe}}
    {
        The compiler executable is imaginatively named \monospace{compiler-exe}, with the single 97-line source file \monospace{Main.hs} residing in the \monospace{app} directory: it contains the application entry point and code for handling command-line arguments, before passing control over to the \monospace{compiler} library for the actual compilation process.
    }
    \subsection{Tests}
    {
